...
- **The toolchains are the machine's, not a pinned set.** `check` runs whatever compiler, linter, and formatter the machine has; `.github/actions/setup-toolchains` installs each one fresh on the runner — `rustup component add`, `go-version: stable`, `node-version: lts/*`, `swift-version: latest`. pre-commit pins what it owns by `rev`, so the drift is confined to these: a check that passes locally and fails in CI is a version difference before it is a flaky runner. Pin the toolchain in the repository that has the language. A pin that ships from a template payload instead obliges every repository generated from it to bump a file for a language it may not have, and Dependabot has no ecosystem for any of them.
...
- **Go capabilities run once per module** through `go_each`, which fails on an empty `go list -m`; gofmt stays at the root. The build discards its output with `-o /dev/null`: a bare `go build ./...` writes an executable wherever the pattern resolves to a single package, naming it after that package's own directory, which for a lone `main` under the Layout rules is `src` and collides with the directory it was built from. A check compiles and writes no artifact; `package` is what delivers one. `_go_main_packages` yields one program, `NO_RUNNER`, or a refusal.
...
- **`libs/dependabot.sh` reads the tracked set, and the gap it finds is a passing row carrying the advisory rather than a finding.** `not-applicable` would be wrong for it: that state says the check did not apply, and here it applied and found something. The detail is what separates the two passing rows. What Dependabot reads is the pushed tree, so it enumerates with `git ls-files` rather than `detect_find`, whose `DETECT_PRUNE_DIRS` was written for root-manifest detection and names no agent directory. It maps the same six manifests the rest of the template detects and no seventh; a shape outside them is reported by a `/repo-builder` flow on its gate, not here. The entries are derived per repository by those flows, so a manifest added since the last update flow owes one — a line for the operator to read, never a red gate on a repository for gaining a module. The derivation rule in `apps/repo-builder/src/references/lifecycle.md` excludes the payload tree at `apps/github-repository-template/src/base-repo/` and this library does not: the file is mirrored into that payload, so an exclusion naming its own path would ship to every generated repository as a rule about a directory none of them has. The two agree while the payload ships no manifest, and a manifest added there is reported here and skipped by the derivation.
...
- `system-packages-test` — an absent manifest, comments and blank lines, a comments-only file, each refusal a line can earn including every apt action suffix, a version-pinned name surviving them, an unreadable manifest, and that `install` reaches `apt-get` with exactly the listed packages and, without a file, reaches it not at all while still naming the manifest in the log

`scripts/check` runs all twenty-one before the checks they guard, then `structure` and `github-parity` before `ci`. `ci.yml` runs them before toolchain setup and installs `pre-commit` first, so the two hook-wiring suites do not skip every case. shellcheck runs via pre-commit with `-x`.
